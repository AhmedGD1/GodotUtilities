...
Also exposes `TryRegister` / `TryRegisterAuto` (non-overwriting variants, id derived from filename), `Unregister`, `Contains`, `Clear`, and introspection via `GetIds()`, `GetPaths()`, `GetMap()`, `GetPath` / `TryGetPath`. Pass a `HashSet<string>` of allowed extensions to the constructor to restrict which files `RegisterFolder` picks up; `.uid` and `.import` files are always skipped regardless.

> `RegisterFolder` derives ids from filenames alone (not full paths), so two files with the same basename in different subfolders will collide — `Register` (and therefore `RegisterAuto`) pushes a warning and overwrites the existing entry rather than failing silently.
