...
public static class UITransition
{
    ...
    private static void OnSlide(Control target, bool isShow, Tween tween, TransitionType transition)
    {
        ...
        if (isShow) tween.TweenSlideIn(target,  direction, SLIDE_DURATION);
        else        tween.TweenSlideOut(target, direction, SLIDE_DURATION);
    }
    ...
    private static void OnScalePop(Control target, bool isShow, Tween tween)
    {
        ...
        if (isShow) tween.TweenScalePopIn(target,  SCALE_POP_DURATION);
        else        tween.TweenScalePopOut(target, SCALE_POP_DURATION);
    }
}   